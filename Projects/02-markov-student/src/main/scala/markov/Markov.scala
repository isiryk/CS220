...
object MyBuilder extends Builder {
  ...
  private[markov] def buildWithLines(lines: Iterator[String],
    ...
    val words = buildWithWords(splitWords(lines.next()), prefix, chain)
    buildWithLines(lines, words._1, words._2)
  ...
  private[markov] def buildWithWords(words: Array[String],
    ...
    val first = getFirstWord(words)
    val suffixx = new Suffix(first)
    val newChain = addToChain(chain, prefix, suffixx)
    buildWithWords(getRestOfWords(words), prefix.shiftIn(first), newChain)
  }
  ...
  private[markov] def addToChain(chain: Chain, prefix: Prefix, suffix: Suffix): Chain = {
    ...
    if (!chain.contains(prefix)) {
      return chain + (prefix -> Vector(suffix))
    }
    val unwrap = chain.get(prefix)get
    val difference = chain - prefix
    difference + (prefix -> addSuffix(unwrap, suffix))
  }
}
...
object MyGenerator extends Generator {
  ...
  private[markov] def generateAll(chain: Chain,
    ...
    val info = getSuffixes(chain, prefix)
    val random = getRandomSuffix(info, rand)
    if(isSuffixNewline(random)){
      return ""
    }
    random.toString + " " + generateAll(chain, words - 1, rand, prefix.shiftIn(random.toString()))
